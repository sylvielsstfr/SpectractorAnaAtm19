...
#----------------------
thedate = "20190215"
spectra_directory = "/Users/dagoret/DATA/PicDuMidiFev2019/spectractor_output_prod4/" + thedate
rawinput_directory="/Users/dagoret/DATA/PicDuMidiFev2019/prod_"+thedate+"_v4"

image_dir="out_cinestarspectrophoto"
image_name="pdmimg"
padding=4
...
if __name__ == "__main__":
    ...
    my_logger.info('\n\tcineStarSpectroPhotoMultiPanels')
    # Load config file
    load_config(config)
    ...
    separation_angle_seq=[]
    for thetime in times_seq:
        angular_sep=starloc.separation(get_moon(thetime))
        separation_angle_seq.append(angular_sep)
    ...
    for idx in np.arange(Nobs):
        ...
        # filename of spectrogram
        thespecfile=thefile.replace(".fit","_spectrum.fits")
        # the filename of reduced image
        therawfile=thefile

        print("{}) : specfile={}, imgfile={} ".format(idx, thespecfile,therawfile))

        fullspectrofilename=os.path.join(spectra_directory,thespecfile)
        fullfilename = os.path.join(rawinput_directory, therawfile)

        basenamecut = thefile.split("_HD")[0]
        ...
        if 1:
            ...
            # show photometry in ax4
            PlotStarmagvsUTC(current_date, current_starmag, current_starmagerr,current_mask, ax4, TMIN, TMAX, Ntot)
            #PlotBkgmagvsUTC(current_date, current_bkgmag, current_bkgmagerr, current_mask,ax2, TMIN, TMAX, Ntot)
            ...
            moon_coord=get_moon(Time(t["date"][idx], format='isot', scale='utc'))
            moon_angsep=starloc.separation(moon_coord)
            current_frame= AltAz(obstime=Time(t["date"][idx], format='isot', scale='utc'),location=site_location)
            star_azimuth=starloc.transform_to(current_frame)

            print(idx,"  ",Time(t["date"][idx], format='isot', scale='utc'),"  " , moon_angsep,"    ",star_azimuth)

            # show Star elevation in ax3
            PlotElevation(delta_midnight, sunaltazs_seq, moonaltazs_seq, staraltazs_seq,DTim ,ax3)

            # show spectrum in ax2

            if 1:
                parameters.obs_name="PICDUMIDI"
                s = Spectrum()
                s.load_spectrum(fullspectrofilename)
                #ax2 = plt.gca()
                labelname = str(idx) + ") :: " + basenamecut + " :: z = {:1.2f}".format(am)

                s.plot_spectrum(ax=ax2, xlim=None,ylim=[0,5e-11] ,label=labelname, force_lines=True,live_fit=True,drawclose=False)




            # already done in    s.plot_spectrum()
            plt.draw()
